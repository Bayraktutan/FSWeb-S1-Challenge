...
function indekseGoreFenomen(arrayOfObjects, index) {
  return `${index}. indekste bulunan fenomen: ${arrayOfObjects[index].profile}`;
}
...
// function profilListesi(someArray) {
...
//     delete obj.posts
//     delete obj.platform
//     returnArray.push(obj.profile)
//   }
//   return returnArray
// }
// profilListesi(fenomenler)

//Alternative Solution Without Using Delete

function profilListesi(someArray) {
  let returnArray = [];
  let newArray = [...someArray];
  for (let i = 0; i < newArray.length; i++) {
    returnArray.push(newArray[i].profile);
  }
  return returnArray;
}
...
function fenomenEkle(array, number, profile, followers, posts, platform) {
  let copyFenomen = [...array];
  let obj = {};
  obj.number = number;
  obj.profile = profile;
...
// function enFenomenler(someArray) {
//   let filteredArray = someArray.filter((obj) => obj.followers > 100000000);
//   let mappedArray = filteredArray.map((element) => element.profile);
//   return mappedArray;
// }

//Alternative Solution Without Using Filter/Map

function enFenomenler(someArray) {
  let returnArray = [];
  for (let i = 0; i < someArray.length; i++) {
    if (someArray[i].followers > 100000000) {
      returnArray.push(someArray[i].profile);
    }
  }
  return returnArray;
}
...
// function fenomenGonderimSayisi(arrayOfObjects, objectProfile) {
//   for (let object of arrayOfObjects) {
//     if (object.profile === objectProfile) {
//       return object.posts;
//     }
//   }
// }

//Alternative Solution Without Using for-of

function fenomenGonderimSayisi(arrayOfObjects, objectProfile) {
  for (let i = 0; i < arrayOfObjects.length; i++) {
    if (arrayOfObjects[i].profile === objectProfile) {
      return arrayOfObjects[i].posts;
    }
  }
}
...
// function platformaGoreCokGonderiYapanFenomen(someArray, somePlatform) {
//   let postCountArray = [];
//   for (let object of someArray) {
//     if (object.platform === somePlatform) {
//       postCountArray.push(object.posts);
//     }
//   }
//   let maximumPost = Math.max(...postCountArray);
//   for (let object of someArray) {
//     if (object.posts === maximumPost) {
...
function platformaGoreCokGonderiYapanFenomen(someArray, somePlatform) {
  let returnArray = [];
  for (let i = 0; i < someArray.length; i++) {
    if (someArray[i].platform === somePlatform) {
      returnArray.push(someArray[i].posts);
    }
  }
  const maximumPost = Math.max(...returnArray);
  for (let i = 0; i < someArray.length; i++) {
    if (someArray[i].posts === maximumPost) {
      return someArray[i].profile;
    }
  }
}
